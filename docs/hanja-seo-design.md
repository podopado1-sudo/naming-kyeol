...
- **빌드 전략 (2026-06-13 구현 확정)**: 전체 9,863페이지를 빌드 타임 생성하면 산출물이 **29,593파일/2.17GB**로 Vercel 한도 위험 → **하이브리드**로 전환.
  - 빌드 타임: 독음 490 + S급 글자 2,054 = ~2,800페이지만 prerender (`generateStaticParams`)
  - 나머지 ~7,000자: `dynamicParams = true`로 첫 방문 시 온디맨드 생성 후 캐시 (데이터는 전부 번들 내 JSON이라 백엔드 의존 없음)
- **단계적 공개**:
