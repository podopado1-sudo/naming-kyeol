...
|---|---|---|---|
| "{음} 한자", "이름에 쓰는 {음} 한자" | "윤 한자 이름", "서 이름 한자" | 독음 페이지 | **767** |
| "{한자} 뜻", "{뜻} {음} 한자" | "潤 뜻", "불을 윤 한자" | 글자 상세 페이지 | **~9,100** |
| "인명용 한자", "인명용 한자 검색" | 한자 사전 진입 | 인덱스 페이지 | 1 |

- **머니 페이지는 독음 페이지** (767개): 부모가 실제로 검색하는 건 "윤으로 시작하는 한자"류. 글자당 최다 133자(유), 평균 ~12자가 한 독음에 묶임.
...
- 초성 ㄱ~ㅎ 탭 → 독음 칩 그리드 (767개를 14개 초성 그룹으로).
...
- 독음 767개: priority 0.6 / 글자: priority 0.5.
...
- **페이지 생성 기준**: `meaning_ko` + 획수 보유 글자만 상세 페이지 생성 (≈9,000자). 둘 중 하나라도 없는 ~500자는 페이지 미생성, 독음 페이지 목록에서 글자+뜻만 표기.
- **단계적 공개**:
  - **1차 배포**: 인덱스 + 독음 767페이지 + S급 오행 글자 2,060페이지만 sitemap 등재. 나머지 글자 페이지는 생성하되 sitemap 미등재 (크롤러가 내부링크로 자연 발견).
  - **2~4주 후** Search Console/서치어드바이저에서 색인율 확인 후 나머지를 sitemap에 추가.
