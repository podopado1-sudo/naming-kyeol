...
- **상표 문서 main 편입**(사용자 지시): `claude/quizzical-mclean-77c5dd`(8/21 커밋 2개)의
  docs/trademark-filing.md를 main에 넣되, 리포가 **공개**라 §2-4의 경쟁 개인사업자 성명·사업자등록번호·
  국세청 주소지는 비식별 처리한 사본을 새 커밋으로 넣었다(브랜치 커밋 자체는 병합하지 않음 — 이력에
  원문이 실리지 않게). 원문은 로컬 브랜치·worktree loving-feistel에 그대로. `claude/wizardly-diffie-9850e3`는
  원격이 삭제된 빈 로컬 브랜치. 희귀 성씨 세션이 남긴 판단 2건은 같은 세션에서 아래와 같이 처리.
- **희귀 성씨 판단 2건 처리**: ① 부정 동음 — 넷 다 출생신고 실명(2008~2019: 인수 479·예정 237·
  원정 225·우수 113명)이고 기존 목록도 실명 수백 명(유신 1,196·아성 267·수용 224)을 포함하므로 기준은
  희귀도가 아니라 "일상어로 읽혀 놀림감·부정 연상". **원정**(원정출산·원정도박)·**예정**("X 예정",
  조용·아주·아무 부류)만 추가, 우수(優秀)·인수(引受)는 유지. ② 둘째 음절 캡 3(`MaxPerSecondSyllable`,
  NamePoolEngine과 동일)을 라운드-로빈 안에 — 캡에 걸리면 같은 첫음절 그룹의 다음 후보로 넘어가
  첫음절 다양성은 그대로. 실측 봉·탁·김 20개 중 '수' 9~12개 → 3개, 첫음절 20종 유지. 테스트 +5
  (Theory 5성씨) → **dotnet test 1,080/1,080**. 캡이 끌어올린 후보 중 인승(N인승, 44명)·우승(優勝, 241명)도
  같은 기준으로 검토해 **유지**(긍정 명사·접미 표현은 놀림감·부정 연상 아님 — 사용자 확인). 명사 충돌 자체를
  좇지 않는다: 우정 585·예선 344·아정 1,444명처럼 실명 다수가 명사와 겹친다. 경계 사례 예선("예선 탈락")만 관찰.
...
**남은 관찰(→ 같은 날 처리, 위 통합 요약의 '판단 2건 처리' 참조)**: ScoreCandidate가 순수 음운 점수라 받침 성씨(봉·탁)에서는 ㅇ초성 첫음절 +
...
**원인**: 대법원 인명용 한자표의 유니코드 미등재 글자를 법원이 자체 코드(`a01b1` 등)로 관리하는데,
수집 단계에서 이 코드를 그대로 코드포인트로 변환 → `hanja_dictionary_final.json`에 미할당 평면
U+A0xxx 377자 + 사용자 영역 U+F0xxx 28자 = **405자**(출처 `gov` 단독, 뜻·획수·오행·부수 전무).
...
- 테스트 +2(케이스 10): `IsHanCodePoint_ClassifiesByUnicodeBlock`, `GetAllHanja_ContainsNoNonHanCodePoints`
  → **dotnet test 1,057/1,057**. 로컬 hanja-stats `withUnicode` 9,190 실측.
- 로컬 dev에서 `/hanja/온` 25카드·비한자 글리프 0·빈 뜻 0 확인

**부수 발견 → 후속 세션(브랜치 `claude/nifty-germain-dee694`)에서 제거**: 하드코딩 상세 사전의
한글 음절 키 항목 10건(우·진·서·하·윤·민·지·현·연·채, 뜻 "하=아래"·"채=채소" 등)은 죽은 플레이스홀더로
판정해 삭제, 가드 테스트의 한글 예외도 제거(모든 키가 `IsHanCodePoint`를 통과해야 함). 삭제 전후 실측:
이름 분석·평가·미학·조화·HanjaSelector·이중 이름 결과 동일, 사전 14,683→14,673자. 다만
`RareSurnameEngine`은 사전 삽입 순서로 풀·한자 옵션을 고르기 때문에 "민(백성)" 같은 가짜 옵션이
사라지는 대신 후보 풀의 서·우·윤·진 음절이 새·섭·예·의로 바뀜(45조합 중 41 변동) — 관련도 정렬로
바꾸는 작업 칩 별도 발행.

---

## 이전 세션 요약 (2026-09-03 — /name 주간 드립 확장 파이프라인 완성·배포)
...
### 점검 결과 (전부 실측)
- dotnet test 1,036 → (수정 후) **1,039/1,039** · 빌드 경고 0 · 상호 변별력 감사 PASS
- 데이터 전 파일 기대값 일치 (name-seo 3,305 / stories 3,289 / comboMeans 16,203 / 오버라이드 95 / creative 3,291 / 한자쌍 126) · combos 감사 A~E PASS
- 세션 커밋 비밀정보 0 · 임시파일 잔재 0 · 라이브 주요 경로 전부 200 · Render warm(0.34s) · keepalive 유효
- 프로덕션 API 오리진은 **naming-kyeol-api.onrender.com** (keepalive.yml이 유일한 단서)

### 말≠동작 결함 5건 수정 (사용자 지시 "거짓말한거 수정해야지" 후속)
같은 부류가 반복돼 부위가 아니라 몸을 검사했다. **출처를 구분해 적는다** —
아래 1·2는 감사를 띄우기 **전에** 직접 찾은 것이고, 3~5만 발화 감사가 잡았다.
(커밋 25c0511의 제목 "발화 전수 감사가 잡은 거짓 5건"은 이 귀속이 틀렸다.
 수정 5건이 그 커밋에 다 들어 있는 것은 맞지만, 감사가 찾은 건 3건이다.)

**직접 발견 2건**
1. "표기는 다르게 풀었어요" ↔ 예약 슬롯이 '카페담'을 앉힘 → 거절 키워드는 생성·가점·예약
   어느 경로로도 재료 금지(`IsRefusedKeyword` — 안내 분기와 같은 기준)
2. "뜻은 살리되" — 반영 실측 0인데 하던 주장 → 문구 삭제, 테스트로 재발 방지

**발화 감사가 발견 3건** — /company의 사용자 발화 2,053건을 3갈래(엔진 생성 발화 216후보
전수 / 프론트 정적 카피 / 적대 입력 스트레스)로 실측 대조, 반증 에이전트가 독립 재현 후 확정.
검증 대상 2,053건 중 거짓으로 지적된 것은 3건(나머지 2,050건은 미지적).
3. [high] '淸' 입력 시 청지(淸智)가 목록에 실재하는데 "넣지 못했어요" 부정 →
   비한글 키워드도 최종 목록(이름+한자 표기)을 보고 발화, 들어갔으면 침묵
4. [medium] 키워드 어근 후보(정성채)가 생성 루프 축("고요 · 여백")을 제 것처럼 서사 →
   어근 후보는 축 루프 밖에서 생성, 뜻="'정성'이 빛나는 곳"(어근+어미 축만), meta=(어미축,어미축)
5. [low] "IT 업종의 '오램' 결" — 톤 주입·편승 축을 업종 소유로 단언(130건 중 73건) →
   소유격 제거("'A'과 'B' 결을 함께 담은 이름이에요"), 조사도 GwaWa로 정리
**원칙을 테스트로 고정**: 말-동작 불변식 8건 (거절 키워드 비노출, "뜻은 살리" 금지,
淸 부정 금지, 어근 정직 서사, "업종의 '" 소유격 금지 등). 테스트 1,047개.

### 점검이 잡아낸 결함 → 수정 완료
1. **[운영 도달 가능] 키워드 안내 거짓 약속 재발** — 결=한자·글자 수=2에서 "'정성'만 따서
   썼어요"라고 말해놓고 목록에 0개 (리뷰 에이전트가 API 실측 재현). 원인: 안내가 선택 결과를
   안 보고 무조건 발화. **BuildKeywordNotices가 최종 목록을 확인한 뒤 문구를 고르도록 수정**
   — 들어갔으면 절단 안내, 한자로 담겼으면 "같은 뜻의 한자로 담았어요", 조건 탓이면 푸는 법 안내
2. **HanjaData 잠복 2건 경화** — ①로드 도중 예외 시 부분 사전이 영구 '완료' 승격(catch에서
   null 롤백) ②빠른 경로 2회 읽기가 Reload()와 TOCTOU(스냅샷 1회 읽기 + lock 안 반환)
3. **shadcn CLI를 devDependencies로** — prod 감사 오탐 high 4건 제거 (8건→4건, 잔여는
   기존 알려진 next 번들 postcss·sharp 계열)

### ⚠️ 다음에 헤맬 함정
- **next/font/google은 빌드타임에 폰트를 다운로드한다** — 점검 중 Google Fonts 일시 장애로
  1차 빌드 실패, 재시도 성공. Vercel 배포 실패 시 재시도부터
- claude/* 로컬 브랜치 2개 + 워크트리 1개(loving-feistel-603966)는 이전 워크플로 잔재 — 무해, 정리 선택
- 부팅 로그의 `DROP COLUMN "BonusNicknames"` ERR 1줄은 멱등 패치의 의도된 실패(SQLite 경로) — 소음일 뿐

### 다음 작업 후보
1. **푸시 + 배포** — 미푸시 커밋이 쌓여 있다(/company 전체 + HanjaData 수정). 백엔드(Render)
   먼저 → 프론트(Vercel ~25-30분). 프로덕션 /company는 3중 확인 결과 순수 미배포 상태
2. 키워드 의미 라우팅(키워드→축 사전, 오프라인 배치) — 기능이 값을 한다고 판단되면
3. 이전 이월: 블로그 발행, 구글+네이버 합산 킬 크리테리아 판정(9월 말)

---

## 이전 세션 요약 (2026-08-28 2차 — 🎛️ 상호 톤·키워드가 결과를 실제로 바꾸게)

사용자 피드백에서 출발: **"담고싶은 말이 11자 밖에 되지않고 조건을 바꿔봤자 결과는 별 다르지 않네"**

### 추측 대신 실측부터
`scripts/audit_company_variation.py`(신규)로 90조합을 돌려 재보니
**업종은 잘 갈리는데(겹침 0~33%) 톤이 무력**했다:
- **6개 업종에서 modern과 playful이 12개 결과 100% 동일** (cafe·food·bakery·wellness·retail·agri)
- law classic/premium 85%, interior warm/playful 85%, it modern/warm 71%

**원인**: 톤은 업종이 고른 4개 축 안에서만 가점(+2.5/축)하고 선호 생성축에 +3 하는 게 전부라,
두 톤의 교집합과 선호 스타일이 같으면 **점수 함수가 완전히 같아진다**.
게다가 100점 중 80점(기억성·발음·식별력)이 사용자 입력과 무관해 전역 순위가 고정돼 있었다.

### 해결 — 톤에 자유도 3개
관점을 나눈 설계 워크플로(독립 3안 → 3렌즈 심사)를 돌려 "서명 축 주입 + 결 쿼터 + 소리결"을 채택.
1. **서명 축 주입** — 톤이 업종 밖 축을 0.55로 재료 풀에 들여온다 (재료 자체가 갈림)
2. **결 쿼터** — 12칸의 한자/우리말/영문 구성비를 톤이 정한다 (클래식 7/4/1, 따뜻함 2/8/2)
3. **소리결** — 같은 재료에서 다른 표기를 고르게 (Memorability 30 중 6점)
...
| 톤 쌍 최악 | 100% | 50% |
| 톤 쌍 평균 | 25~37% | 6~16% |
| 감사 전체 실패 | 20건 | **0건 (PASS)** |

카페 예시 — modern `루멘 새빛 노벤 솔라` / classic `유직悠織 구봉久峰 온혜溫惠 담예澹藝` /
warm `포근담 열매온 이삭온` / playful `루콘 카론 돌카 노을담`

### 키워드 3갈래
어미 절단(정성스러운→정성) / 한자 음절 매칭(지혜→智·慧·惠 든 검수쌍) / 반영 못 하면 안내.
길이 상한 10→20자(사용자가 직접 불평한 항목이라 올렸다).

⚠️ **말과 결과가 어긋나던 결함을 잡았다** — "'정성'만 따서 썼어요"라고 안내해놓고
결과에 정성이 든 이름이 하나도 없었다. 점수 가산(+2)으로는 12칸 경쟁에 밀려서,
`SelectDiverse`가 **키워드 적중 후보에 한 자리를 예약**하도록 고쳤다.
아무 말 안 하는 것보다 나쁜 상태였다.

### 덤으로 고친 것
- 업종 축 재배치 8건 — `finance/law`가 축 4개 중 3개를 공유해 **결과 100% 동일**이었다
  (변경 전에도 그랬는데 측정을 안 해서 몰랐다). food/bakery, it/edu, it/consulting도 해소
- 조사 오류(`'정성스러운'는` → `은`) — `KoreanUtils.EunNeun` 재사용

### ⚠️ 남은 한계
업종 쌍 겹침 최대 **71%** (`food/agri`, `fashion/culture` 등). 축 12개로 업종 18개를
표현하는 구조적 한계라 재배치로는 더 못 줄인다. 감사 게이트를 0.72로 두고 근거를 주석에 남겼다
— 이 게이트의 목적은 이상적 목표치가 아니라 "오늘보다 나빠지는 것"을 잡는 회귀 방지다.

### 후속 정정 — 키워드 약속을 실제 능력에 맞춤 (`83d0bb1`)
사용자가 "담고싶은말을 어떻게 해야할지 의구심"을 제기 → 16개 일반 단어로 실측하니
**12개 중 1개만 바뀌고 그 1개가 예약 슬롯**이었다. 키워드→축 의미 라우팅이 없어
'정직' 같은 말은 아무 축에도 안 걸린다(축 매칭은 substring 우연). 라벨이 과장이었다.
- "담고 싶은 말" → **"이름에 넣고 싶은 글자"** — 리터럴 보장만 약속
- "뜻으로만 반영했어요" 안내 삭제 — 보장 못 하는 말은 하지 않는다
- 뜻 반영은 후속 후보로: **키워드→축 사전을 오프라인 LLM 배치**로 생성
  (combo-meanings 16,203쌍과 같은 패턴, 런타임 비용 0). 기능이 값을 한다고
  판단된 뒤에 착수할 것 — 사전부터 만들면 순서가 뒤집힌다
...
## 이전 세션 요약 (2026-08-28 — 🏢 회사 상호 작명 `/company` 출시 + HanjaData 경합 수정)

`CLAUDE.md`에 "준비 중"으로 남아 있던 **회사명**을 실제 기능으로 해소한 세션. 커밋 전 상태.

### 범위 (사용자 확정)
v1 = **엔진 + 화면만**(외부 API 0) / 생성 축 **3종 전부** / 타깃 **가게·브랜드명 포함**.
도메인 가용성·상표 조회(KIPRIS)는 Phase 2/3으로 분리.

### 왜 별도 엔진인가
성씨가 없어 `AestheticEngine`의 성+이름 연음 평가(30점)가 성립하지 않고, 인명용 한자
제약도 없으며, 평가축이 미학/조화가 아니라 **기억성 30 + 발음 25 + 식별력 25 + 업종적합 20**이다.
식별력이 상호만의 축 — 상표법상 기술적 표장은 등록이 어렵고 검색에서도 묻힌다.

### ⚠️ 구현 중 방향을 바꾼 지점 — 한자 축은 생성이 아니라 큐레이션
한자 2자 자유 순열을 돌렸더니 도달 가능 표기가 **1,797개**였고 그 안에
`久續=구속` `工匠=공장` `安靜=안정` `素明=소명`이 섞여 나왔다. **법률사무소 후보로
'구속'이 뜨는 사고는 감점으로 못 막는다.** 한국어는 음절이 적어 임의 조합이 기존
한자어와 동음이 될 확률이 구조적으로 높고, 저장소에 쓸 한국어 사전도 없다.
→ **검수 한자쌍 140개**로 교체(이후 검수에서 14쌍 제외 → 126개). 메모리 [[project_naming_engine_quality]]의
"창의 이름은 생성 < 큐레이션"과 같은 판단. 순우리말·영문 축은 생성 유지.
**쌍을 추가할 때 한글 표기가 기존 단어와 겹치는지 반드시 확인할 것.**

### 그 밖의 품질 교정 (전부 실측 후 수정)
- 순우리말 뒷자리를 검수 어미 15개로 제한 — `골깨움`·`볕헤아` 류 제거 (833 → 552)
- 라틴 어근에서 `sophi`→소파, `sapi`→사포, `gemma`→겜마, `copia`→코피아 제거
- 다양성 선택 3단 sweep — 법률에서 `구○`만 8개 나오던 쏠림 해소
- **생성 축 상한 1/3** — 절반으로 뒀더니 한자가 전 업종에서 0~2개로 밀렸다(실측
  `cafe {english:6, pure-korean:6}`). `ceil(count/3)+1`로 낮춰 **5/5/2** 확보
- 후보별 `Cautions`는 실사용에서 **0건**(감점이 커서 상위 진입 불가) → 응답에
  `KeywordNotices` 신설. 업종어를 키워드로 넣으면 "왜 안 썼는지"를 결과 위에서 안내

### 🔴 HanjaData 경합 수정 (계획 밖이었지만 필수)
새 테스트 24개를 넣자 전체 스위트에서 **14건이 깨졌다**. 기준선(내 코드 제외) 988/988
통과 확인 → 내 변경이 유발. 원인은 내 코드가 아니라 **기존 이중검사 잠금 결함**:
`HanjaData.cs:702`가 `_loadedDictionary`를 채운 **뒤** 706행에서 외부 데이터를 병합하는데,
프로퍼티 빠른 경로는 그 필드를 **lock 밖에서** 읽는다 → 병합 전에 들어온 스레드가
Core_v1·인명빈도가 빠진 사전을 들고 나간다. 705행 주석("lock 안이라 부분 로드를 볼 수
없다")은 사실과 달랐다.
**수정**: `volatile bool _fullyLoaded`(독자 게이트) + `[ThreadStatic] _loadingOnThisThread`
(로더 재진입 허용) + `Reload()`에서 플래그 동반 해제.

### 검증 (전부 실측)
- `dotnet test` **1,018/1,018** (경합 수정 확인용으로 1012 상태에서 3회 연속 통과 확인)
- 프론트 `tsc --noEmit` · `lint` · `npm run build` 클린, `/company` 정적 프리렌더
- 브라우저 실동작: 12개 렌더 / 축 분포 5·5·2 / 키워드 '커피' → 안내 문구 노출 /
  홈 카드 → `/company` 이동 / `/guide` 7장 링크 / 모바일 375px 가로 오버플로 0

### 🔍 커밋 전 적대적 검수 — 한자쌍 140 → 126

손으로 고른 목록이라 놓친 게 있을 수 있어, 커밋 전에 워크플로로 털었다
(6개 청크 독립 검수 → 렌즈 3개 반증 → 2/3 다수결). 42건 지적 중 10건 확정.

**여기서 처음 드러난 결함 유형: 표기와 발음이 갈라지는 쌍.**
받침 뒤에 ㄹ초성이 오면 유음화·비음화가 예외 없이 일어난다 —
`溫林 "온림" → [올림]`, `旭林 "욱림" → [웅님]`, `溫隣 "온린" → [올린]`.
상호는 듣고 받아적어 검색하는 이름이라, 전화로 불러주면 손님이 다른 글자를 적는다.
⚠️ 검수 쪽 판정이 갈렸다(같은 논리인데 `담린·송린·승린`은 기각, `담림`은 미지적).
**에이전트 결과를 그대로 받지 않고 부류 전체를 제거**했고,
`HanjaPairs_NoLiquidNasalShift` 테스트로 재발을 기계적으로 막는다.

제외 14쌍: 발음변동 7(溫林 旭林 澹林 溫隣 澹隣 松隣 承隣) /
고유명사 2(澹原=담원게이밍, 燦原=이찬원) / 기존어휘 2(素潭=소담하다, 久智='굳이' 오기) /
근접동음 3(智恒=지향, 峰恒=봉황, 澹豊=단풍)

**검수와 별개로 직접 잡은 품질 결함 3건**
- 한자 슬롯 2개가 둘 다 같은 글자로 시작(`서각/서아`) → 축별 첫 글자 상한 추가
- 뒤집은 쌍이 나란히 노출(`지결/결지`, `마루채/채마루`) → 구성요소 집합 키로 1차 제외
- `채`가 앞자리에 오면 말이 안 됨(`채길`·`채결`) → 뒷자리 전용으로 변경

### ⚠️ 다음에 헤맬 함정
- dev 서버는 `UseHttpsRedirection`으로 HTTP를 **307** 리다이렉트 → curl은 `https://localhost:5001`
- Git Bash에서 한글 JSON을 `-d`로 인라인 전달하면 인코딩이 깨진다 → 파일로 `--data-binary @file`
- 임베디드 브라우저에서 한글 타이핑(`computer type`)이 입력창에 안 들어간다 →
  네이티브 setter + `input` 이벤트로 주입할 것
- `dotnet build` 전에 실행 중인 `NameForm.exe`를 죽여야 한다 (파일 잠금 MSB3027)
- **셸 파이프로 한글이 깨진다** — curl 응답을 파이프로 python에 넘기면 surrogate 오류.
  파일로 받아(`-o`) python이 직접 읽게 할 것

### 다음 작업 후보
1. 커밋 — ⚠️ 다른 세션의 `docs/naver-blog/*`가 섞이지 않게 내 파일만 명시 스테이징
2. `/company` Phase 2: 도메인 가용성(RDAP/DNS) → Phase 3: KIPRIS 상표 간이 확인(면책 카피 선행)
3. 상호 SEO 정적 페이지 검토 ("업종별 상호 예시") — 단 구글 재평가 기간 고려
4. 이전 세션 이월 항목(블로그 발행, 8/18 중간 체크포인트 등)은 아래 참조

---

## 이전 세션 요약 (2026-07-30 2차 — 🔒 Supabase RLS 전면 활성화 + 부팅 자동화)
