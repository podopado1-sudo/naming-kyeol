...
## 마지막 세션 요약 (2026-07-02 — 🈶 대표 훈 오버라이드 + Weak 366자 확장 + 호환자 차단 회귀 수정)

핸드오프 후보 2·3을 결합 진행. 커밋 3개: `d0f10bf`(NFKC) → `d422481`(대표 훈) → `26a4047`(Weak).

### ① 호환자 차단 회귀 수정 (`d0f10bf`) — 작업 중 발견한 라이브 버그
- 지난 세션의 호환자 8종 리터럴이 **편집 과정의 NFC 정규화로 일반 코드포인트 중복으로 치환**
  → 사전의 CJK 호환 엔트리(U+F918 落, U+FA19 神 등 8자)가 실제로는 미차단이었음.
- `IsForbiddenNameHanja`를 NFKC 정규형 2차 조회로 변경(향후 모든 호환 변형 자동 차단) + 중복 리터럴 제거.
- ⚠️ **교훈**: 호환 코드포인트는 리터럴로 쓰지 말 것 — 회귀 테스트도 정수 코드포인트(`char.ConvertFromUtf32`)로 작성함.
- 당시 combos에 호환자 노출 0 → 데이터 재생성 불필요였음.

### ② 대표 훈 오버라이드 (`d422481`) — 표시 품질
- **`data/hanja-gloss-overrides.json`(95자) 신설**: 사전 첫 훈이 통용 훈이 아닌 글자를 로드 시 재배열
  (然 불탈→그럴 연, 俏 거문고 탈 소→닮을 초, 朴→성씨 박, 蔚→우거질 울 등. 대부분 표기 독음≠첫 훈 음 오류).
- `HanjaData.ApplyGlossOverrides`는 **반드시 `LoadStrokeData()` 직후** — `hanja_meanings.json`이
  Meaning을 무조건 덮어쓰므로(:1334) 모든 기록자 이후여야 함. `ReorderGloss`는 이동/앞삽입·무손실·멱등.
- 소비처 5곳(CleanGloss·BuildCardMeaning·dump-combo-glosses·firstGloss·BuildMechanicalMeaning)은
  전부 `Split(',','/',';','·')[0]`이라 무수정 전파. `build_hanja_seo_data.py`에도 동일 적용(+hanja-seo 재생성).
- combo 뜻은 **새 훈과 모순되는 42건만** 인라인 윤문(然 불꽃 33 + 舜 무궁화 9) — 蔚·宅·商 쌍은 기존 뜻이 정합해 유지.
- 검수: 전수 스캔 2,405자(다중 훈) → 서브에이전트 triage 6개 → 사용자 확정(노출 7자 개별).
- 鰥(홀아비 환)은 정확한 대표 훈이 부정이라 오버라이드 대신 불용 추가.

### ③ WeakGivenNameHanja 15→366자 (`26a4047`) — 감점 확장
- 세트를 HanjaSelector→**HanjaData.WeakGivenNameHanjaSet 이관**(+접근자, Selector는 위임 심).
- 전수 스캔(약한 훈 사전: 나물·도구·신체·허사 등 9개 카테고리) → 검수 확정 345자 + 재생성 보충 7자.
- **감점이라 combos 소실 0** — 186+개 이름 조합 개선(菜→彩, 焉→言, 腸→壯, 脅→協, 한얼→韓乻).
- 소비처 보강: ExplanationEngine 폴백(weak 후순위 안정 정렬) + NamePoolEngine(-30).
- 신규 240쌍 인라인 윤문 → **comboMeans 100%(12,477쌍) 유지**.
- 검증: dotnet test 982/982(+29), 프론트 prod 빌드 에러 0. **push는 보류** — 사용자 확인 후 배포.

### 핵심 교훈 / 주의
- **weak 두더지 패턴**: 약한 글자를 밀면 얇은 풀에서 더 나쁜 글자가 승격됨(糵 누룩↓ → 孼 서자↑ →
  臬 말뚝↑). 재생성 diff 검수를 2회 반복해 수렴. 보충: 蘖窣臥賽未鰓臬 weak + 孼 불용.
- **경계(hold) 24자 의도적 미포함** (재스캔 시 재제안 금지): 冶 豆 貝 伊 汝 掖 柄 砥 礎 礪 禾 穀 膽 芯
  萊 蓬 藻 袖 襟 軸 錐 頤 駕 鯉 — 통용 의미가 긍정 연상(도야·낭중지추·등용문·흉금 등). 微(작을)·屢(여러)도 무해 판정.
- **오버라이드는 재배열이지 삭제가 아님** — 원 훈 전부 보존되므로 사전 상세 페이지 정보 손실 없음.
- 스캔 도구는 이번에 커밋함: `scripts/scan_weak_name_candidates.py` (약한 훈 사전 포함, 재실행 가능).
- 잔존 니트: 賽論·鰓別 등이 대안 부족 음절에서 4위 조합으로 남음 — soft-yield 설계 의도(소실 방지).

### 후속 (같은 날): 商·貨·暈 weak 추가 — ✅ 완료
- 商 검토 중 **점수 아티팩트로 음절을 점유하던 글자 2개 추가 발견**: 貨(재물 화)가 화 음절
  8쌍(라이브에 평화=平貨·화평=貨平·미화=美貨!), 暈(무리 훈)이 훈/운 음절 55쌍(재훈=在暈·태훈=太暈).
- 3자 weak 추가 → 108개 이름 개선: 평화→**平和**, 화평→和平, 재훈→才訓, 태훈→泰訓, 성훈→星訓,
  남훈→南薰, 훈민→訓敏, 상우→祥佑, 상현→常弦 등. 소실 0, 신규 131쌍 인라인 윤문, 커버리지 100%(12,481쌍).
- 교훈: **대표 훈 정정은 weak 후보를 드러낸다** — 商(헤아릴→장사)처럼 정정 후 훈이 사물/상업이 되면
  대안(尙常相祥/和花華/勳薰訓) 존재 확인 후 감점 검토가 정석.

### 후속 2 (같은 날): 음절 점유 아티팩트 전수 점검 — ✅ 완료
- **`scripts/audit_syllable_occupancy.py` 신설**: combos 점유 글자 727자 전수 감사 →
  서브에이전트 triage 2개 → 어색 훈 확정. **weak 4라운드 반복 수렴**(1차 184 + 2차 36 + 3차 22 + 4차 10 = 252자,
  세트 총 621자). 라이브 심각 사례 다수 해소: 소망=素**忘**→素望, 축복=**逐**福→**祝福**, 행복=行馥 정리,
  혁 음절 181쌍 革(가죽)→侐·爀·奕, 嬪(아내)→彬·斌·賓, 言→彥 등. 총 1,400+개 이름 개선, 소실 0.
- **[근본 원인 수정] Core_v1 가점(+2000)이 weak 감점(-1000)을 압도**하던 버그:
  코어셋은 오행 '검수' 커버리지라 불용·약자도 포함되는데, 그 신뢰도 점수가 조합 품질 경쟁을 이김
  (革이 赫을 이기던 이유). → ComboBaseScore·BestGloss 감점 **-1000→-3000** 상향.
- 윤문: Batch API 2회(2,467 + 578쌍, Sonnet ~$0.8) + 아스트랄 4쌍 인라인(𠃗𩇣𧗿 —
  dump-combo-glosses의 UTF-16 함정으로 배치 누락, 핸드오프 경고 그대로 재현됨). 커버리지 100%(12,459쌍).
- 잔존 니트: 극박 음절 4위 슬롯의 啣·枙·賦 등 — soft-yield 설계 범위로 종결.

### 다음 작업 후보
1. (d) /name 미학/발음 점수 섹션, 공유 OG 카드 (후순위, 이월)
2. /hanja 색인 추이 + 단계적 공개 2단계 (기존 이월)
3. ~~NamePoolEngine weak 감점(-30) 재검토~~ ✅ 완료(같은 날) — -3000 상향. 영향 발음 10개(논농랍론롱별솔와즉혁)의
   대표가 비약자로 교체되어 오행/성별 가산이 실제 배정 한자와 정합. HanjaSelector 쪽은
   '빈출셋 유일-약자 붕괴' 발음 0건으로 안전 확인(풀 게이팅 + -30으로 충분).
