...
### 🔴 Vercel 402 사건 (해결됨)
- **원인**: Hobby 한도 3중 초과 — Edge Requests **3M/1M**·ISR Reads 2.5M/1M·Fast Origin
  Transfer 17.43/10GB → 계정 Paused, 전 페이지 402(DEPLOYMENT_DISABLED). 19.5K 정적 페이지
  +크롤러 트래픽이면 **구조적으로 Hobby 불가** — 사고 아닌 성장통.
- **해결**: 사용자 Pro 업그레이드($20/월, 팀 단위라 examcal 포함). 라이브 재검증 전항목 200
  (서연 서사 렌더·OG png·sitemap). Pro는 정지 대신 종량 과금 → **Spend Management $10
  알림(자동정지 Off)** 설정 완료. 포함 크레딧 $20 내 소화 전망(실측 $0.34/일).
- **실피해 없음**: 구글 크롤 응답 90일 4xx 0건·네이버 수집 오류 0 — 다운은 짧았음
  (OG 캐시 Age 역산 ~7/27 이후 시작 추정).
- **재발 방지**: keepalive.yml 개편(`9287a2a`) — 프론트 다운 감지 추가(3회 재시도, 실패 시
  run 실패→GitHub 알림 메일), 백엔드 ping도 실패 처리화(기존은 warning만 찍고 성공 처리라
  알림 불가), workflow_dispatch 실검증. ⚠️ 실측: GitHub cron 스로틀링으로 10분 설정이
  **약 1시간 간격**까지 밀림 → Render cold start 완전 회피 실패(26초 실측). 근본 해결은 외부 핑어.

### 점검 결과 (전부 실측): 코드·데이터 전부 건강
- dotnet test 988/988 · 프론트 tsc/lint/빌드 클린 19,524유닛 전량 정적 · combos 감사 A~D PASS ·
  데이터 카운트 전부 기대치 일치(stories 3,289 / name-seo 3,305·서사 3,289 / comboMeans 16,203 / 오버라이드 95)
- DB(Supabase) 생존 + Render 최신 코드 확인 (usage/summary 실측). **usage 6주 실사용 ~4.5천 건**
  — evaluate 2,420 > smart 1,132 > required-char 631 (평가가 추천의 2배)
- creative-name-meanings는 2,480이 아니라 **3,291**이 현행(e41f66f 확장분 — 문서 기대값이 낡았던 것)

### 보안·정리 커밋
- `3478ad3` 백엔드: **SQLitePCLRaw bundle 2.1.12**(GHSA-2m69-gcr7-jv3q 해소 — advisory 페이지
  "Patched: None" 표기는 무시할 것, 2.1.12가 실제 패치·dotnet audit 클린) + CS8602(35행 불필요
  `?.` 제거) + xUnit1026 반쪽 테스트 실단언화(리플렉션으로 private 분류기 검증)
- `8ca3282` 프론트: **next 16.2.12** + audit fix → 취약점 15(high 9)→3. 잔여 3건은 next 번들
  postcss 8.4.31 고정·sharp ^0.34.5 탓 — **next 다음 릴리스에서 재확인** (force 다운그레이드 금지)
- `b8180e1` CLAUDE.md 5건 현행화 / 병합 브랜치·메모리 고아 2개 정리
- `5d85507` (칩→별도 세션): 분류기 긴 키워드 우선 매칭 — '용기'·'지혜' 오분류 해소

### 📈 SEO 판독 — 세션 최대 발견: 네이버가 주채널
- **구글**: 색인 3,700(7/15의 2배)·크롤됨-미색인 115→43·발견됨-미색인 0 — 8월 초 관찰 목표
  합격. 단 실적은 7/20 정점(일 노출 3.8천) 후 **허니문 급락 → 재평가 중**(4~12주 관찰, 회복은
  기본값 아님. 재평가 중 구조 변경 금지).
- **네이버(서치어드바이저 실측)**: 30일 클릭 **8.4천 = 구글의 65배**, 노출 30.7만·CTR 2.8%,
  색인 3,430 우상향, 일 노출 23K+. ⚠️ 요약 차트 y축에 클릭이 눌려 "0"으로 보임 — 반드시
  "자세히 보기"로 실수치 확인(이번에 오판할 뻔). 상세는 메모리 [[project_naver_primary_channel]].
- **킬 크리테리아 수정**: 구글 단독 → **구글+네이버 합산**. 중간 체크 8/18, 판정 9월 말.
- 병목 = 노출→클릭(통합검색에서 웹사이트 영역 하단) → 공략 무기가 블로그.

### 📝 네이버 블로그 가동
- **1편 발행**: https://blog.naver.com/podonabi1/224362932645 — podonabi1 빈 블로그를
  "이름 짓는 사람"으로 재단장(계정 연식+무이력 = 최적). 점검: 이미지 1장·키워드 2회로 수칙
  미달(부족 방향이라 무해) — 발행 직후 수정 금지 원칙, 2편부터 준수.
- **3·4편 초안 실측 키워드 재조준**(`bda95e1`): 3편 신흥 이름 클러스터(로아·하엘·로운·하온·태오
  — 전부 실측 TOP 30 검색어. **하온 남녀 48:52 완전 중성** 발견), 4편 "이름에 쓰는 한자 정"
  (실측 **1위** 검색어 CTR 185%, 한자 배정 빈도 실측 6자 晶靜正情精貞 — 시리즈 1탄, '진'=5편 후보).
  strategy.md에 발행 로그 신설. 원안 캘린더 3~8편은 순연.

### 다음 작업 후보
1. **블로그 2→3→4편 발행** (사용자, 주 2회 — 이미지 2장·키워드 4회 준수). 발행 URL 받으면
   5편+('진' 등 서치어드바이저 유입 검색어에서 다음 음절 선정)
2. **다음 주 초**: 서치콘솔 크롤링 통계 4xx 재확인(지연 데이터 반영분) + 실적 페이지 탭
   `/name/` 필터로 이름 뜻 페이지 노출 진입 확인
3. **8/18 중간 체크포인트**: 구글 노출 회복 여부 + 네이버 유입 추이 (킬 크리테리아 중간 판독)
4. 봇 트래픽 분석 → Vercel Firewall 규칙 (Pro 주기 데이터 며칠 쌓인 뒤 — 이제 목적은 정지
   회피가 아닌 비용 절감·examcal 몫 확보)
5. next 패치 릴리스 추적 — 잔여 취약점 3건(postcss·sharp) 해소 여부
6. (선택) keepalive 외부 핑어 이관 검토 — GitHub cron 스로틀링 근본 회피

---

## 이전 세션 요약 (2026-07-22 — ✒️ 의미 코이닝 Phase A: 사람 서사형 story 레이어 + /creative 버그)

창의 후속 #3(2026-06-24 이월) **완결**. 이름이 그리는 사람상 한 문장을 mean과 별개 story
레이어로 추가 — 창의 카드·TopPick·/name Hero 인용 블록에 노출. Phase A(코드)+Phase B(배치
3,289개 생성·병합)까지 완료. 커밋 `b701ce9`(Phase A) `9a2a027`(분위기 힌트) `f2b44ad`(데이터).
...
- 프롬프트 품질 시험 30개(sonnet --sync, ~$0.05): 시험 1차에서 ① '고요/은은' 편중(13/30)
  ② 완전 동일 문장 중복(가나=가빈) 발견 → 사용자 승인 하에 분위기 힌트 추가 후 전량 배치

### 후속 (같은 세션): 📝 네이버 블로그 연계 킥오프
SEO 2순위 전략 착수. 사용자 확정: 기존 계정+새 블로그 / **만드는 사람 개인 톤** / 주 2회 /
**이름 데이터 시리즈 단일 기둥**. `docs/naver-blog/strategy.md`(2026 로직 조사·저품질 수칙·
키워드 티어·4주 캘린더 8편·UTM 퍼널) + 첫 초안 2편(`posts/01·02`, name-seo.json 실측 데이터).
**다음**: 사용자가 블로그 개설·초안 다듬어 발행 → 발행 URL 받으면 3·4편 초안(캘린더 참조).
핵심 수칙: 외부링크 글당 1개, AI 초안 그대로 복붙 금지(다듬기 가이드 초안에 내장), 원본 이미지 2장.

### Phase B 완료 (같은 세션, 사용자 승인 후)
- **분위기 힌트 로테이션**(`9a2a027`): 팔레트 8종 + 어휘 앵커를 이름별 결정적 주입.
  ⚠️ **해시 교훈**: `h*31+ord` 곱셈 해시는 한글 코드포인트 구조(가≡0 mod 8, 나·빈 동일 잔차)와
  맞물려 동음 계열이 같은 버킷에 뭉침 → **md5(솔트로 축 분리)**로 교체. 라벨만 주면 모델이
  상투어로 회귀 — 어휘 앵커(환한·경쾌한·싱그러운 등)까지 줘야 실제로 흩어짐.
  재시험 30개: 고요 계열 5/30, 중복 0.
- **전량 배치**(`f2b44ad`): 3,289/3,289 성공·오류 0 (~20분 처리). 스팟체크 — 중복 27건(0.8%),
  금지 패턴 0, 길이 중앙값 25자, 종결 사람 70%:변형 30%, 고요 계열 17%.
- name-seo.json 1.51→1.74MB(서사 99%). dotnet test 988/988(실데이터 정합), prod 빌드
  19,524유닛 에러 0. **push·배포·라이브 검증 완료** — push 21분 시점 반영 확인,
  /name 서연·도현·라영·여울(서사 표시)+소피아(숨김) + /creative 실동작(버그 수정 라이브,
  카드·TopPick 서사, Render 백엔드 데이터 포함 배포)까지 스팟체크 통과.
  story 미보유는 16개뿐(제이든·소피아 등 외래명 — 한자 글로스 없음, 정상 숨김).
...
| 커밋 | 내용 |
|------|------|
| `6a9b8fe` | **`scripts/audit_combo_regressions.py`** — 재생성 직후 원커맨드 감사(`--base REF`): A combos 소실 / B 불용 잔존 / C comboMeans 커버리지(코드포인트) / D 빈출셋 유일-약자 붕괴(HanjaSelector 게이팅 재현) → FAIL exit 1 + E 두더지 diff(신규 승격 글자·훈·weak표시·샘플) |
| `f9c0ecb` | CLAUDE.md 현행화 — weak 절차를 신규 스크립트 중심으로 재기술, /name 데이터 재생성 순서(combos→scores→build→glosses + og 폰트) 신설, 라우트 섹션 19,524유닛 기준 갱신 |

### 검증
- 클린 상태: A~D 전부 ✅ (특히 D 붕괴 0건 — 지난 세션 수작업 결과와 일치, 정의 재현 확인)
- 검출력: `--base cde9f71^`로 대수술 이전 대비 실행 → E에 승격 129자(飛·際·普 등) 정확 검출
- 잘못된 ref → A/E 경고 후 스킵, B/C/D만 수행 (graceful)

### 남은 작업
1. /hanja·/name 색인 추이 재확인 — **8월 초** (7/15 전체 공개 2~3주 뒤).
   D급 '크롤링됨-미색인' 누적 여부 + /name 색인 진입 + 공유 카드 후 CTR 변화 관찰
2. (상시) weak 추가 시: 재생성 → `audit_combo_regressions.py` → E 두더지 검수 → 윤문 → 재실행 100%

---

## 이전 세션 요약 (2026-07-21 — 📊 /name 미학 점수 섹션 + OG 공유 카드 3,305장)
...
### 다음 작업 후보
1. ~~배포 확인~~ ✅ 완료(2026-07-22) — og·twitter 이름별 카드 라이브 검증 + 실제 카카오톡
   공유 테스트로 서연·주원 카드 정상 확인(사용자 스크린샷). 캐시 퍼지 불필요했음
2. /hanja·/name 색인 추이 재확인 — sitemap 전체 공개(7/15) 2~3주 뒤 = **8월 초**.
   D급 '크롤링됨-미색인' 대량 누적 여부 + /name 색인 진입 여부
3. ~~weak 추가 시 정기 절차화~~ ✅ 완료(2026-07-22) — `audit_combo_regressions.py` 원커맨드 감사
