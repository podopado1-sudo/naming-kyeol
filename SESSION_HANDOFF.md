...
### 발화 전수 감사 (사용자 지시 "거짓말한거 수정해야지" 후속)
같은 부류(말≠동작)가 4번 재발해 부위가 아니라 몸을 검사 — /company의 사용자 발화
**2,053건**을 3갈래(엔진 생성 발화 216후보 전수 / 프론트 정적 카피 / 적대 입력 스트레스)로
실측 대조, 지적은 반증 에이전트가 독립 재현 후 확정. **거짓 5건 발견 → 전부 수정**:
